...
[source,cypher]
----
CREATE (david:Person{name:'Dave Petrocelli'})
CREATE (fcen:Experience{name:'Manager of Latin America Operations for Financial Crimes Enforcement Network'})
CREATE (citi:Experience{name:'Staff Attorney at Citigroup, Commercial Banking Division'})
CREATE (david)-[:HAS_EXP]->(fcen)
CREATE (david)-[:HAS_EXP]->(citi)
CREATE (fcen)-[:IS_SIMILAR{degree:19}]->(digitalcurrency:Knowledge{name:'Digital currency'})
CREATE (fcen)-[:IS_SIMILAR{degree:15}]->(moneylaundering:Knowledge{name:'Money laundering'})
CREATE (citi)-[:IS_SIMILAR{degree:23}]->(financialservices:Knowledge{name:'Financial Services'})
----
...
CREATE (toshi:Person{name:'Toshi Sato'})
CREATE (terrorismfinance:Experience{name:'Managed development of a terrorism financing analysis system'})
CREATE (toshi)-[:HAS_EXP]->(terrorismfinance)
CREATE (terrorismfinance)-[:IS_SIMILAR{degree:19}]->(moneylaundering:Knowledge{name:'Money laundering'})
----
And render as a graph.
